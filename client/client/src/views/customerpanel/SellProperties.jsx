// // client/src/views/customerpanel/SellProperties.jsx
// import React, { useEffect, useState } from "react";
// import { MapPin, Eye, Loader, Search } from "lucide-react";
...
//   const variants = { default: "bg-blue-600 text-white hover:bg-blue-700", outline: "border border-gray-300 bg-white text-gray-700", ghost: "bg-transparent text-gray-600" };
...
//           <input placeholder="Search sale properties..." className="pl-10 pr-3 py-2 border rounded-md w-full" />
//         </div>
//       </div>
...
// client/src/views/customerpanel/SellProperties.jsx
import React, { useEffect, useState } from "react";
import { MapPin, Eye, Loader, Search, Heart, CheckCircle, Trash2 } from "lucide-react";
import API from "../../Api/axiosConfig";
...
export default function SellProperties() {
  ...
  const fetchSell = async () => {
    ...
    try {
      ...
      console.error("Sell fetch error", err);
      setToast({ message: "Failed to load sale properties", type: "error" });
  ...
  const toggleSave = async (id) => {
    ...
    try {
      ...
      setToast({ message: err?.response?.data?.message || "Unable to update saved list", type: "error" });
    } finally {
      setTimeout(() => setToast({ message: "", type: "" }), 3000);
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="w-80 relative">
          <Search className="absolute left-3 top-3 h-4 w-4 text-gray-400" />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search sale properties..." className="pl-10 pr-3 py-2 border rounded-md w-full" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {list.map((p) => (
          <Card key={p._id} className="overflow-hidden">
            <img src={placeholderImg(p)} alt={p.title} className="w-full h-44 object-cover" />
            <CardContent>
              <h3 className="font-semibold text-lg">{p.title}</h3>
              <p className="text-sm text-gray-600 flex items-center"><MapPin className="mr-2 h-4 w-4" />{p.location}</p>
              <div className="mt-3 flex items-center justify-between">
                <div className="text-lg font-bold">{formatCurrency(p.price)}</div>
                <div className="flex gap-2">
                  <Button onClick={() => toggleSave(p._id)} variant="ghost" size="sm">
                    {savedIds.includes(p._id) ? <span className="flex items-center gap-1 text-red-600"><CheckCircle className="h-4 w-4" />Saved</span> : <span className="flex items-center gap-1"><Heart className="h-4 w-4 text-gray-600" /> Save</span>}
                  </Button>

                  <Button onClick={() => window.alert("Open details")} variant="default" size="sm">
      ...
      {toast.message && (
        <div className={`fixed bottom-6 right-6 rounded px-4 py-2 shadow ${toast.type === "error" ? "bg-red-600 text-white" : "bg-green-600 text-white"}`}>
          {toast.message}
        </div>
      )}
    </div>
