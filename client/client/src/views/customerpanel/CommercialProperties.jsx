// // client/src/views/customerpanel/CommercialProperties.jsx
// import React, { useEffect, useState } from "react";
// import { MapPin, Eye, Loader, Search, Building2 } from "lucide-react";
...
//         <h2 className="text-2xl font-semibold">Commercial Properties</h2>
//         <div className="w-72 relative">
//           <Search className="absolute left-3 top-3 h-4 w-4 text-gray-400" />
//           <input placeholder="Search commercial..." className="pl-10 pr-3 py-2 border rounded-md w-full" />
//         </div>
//       </div>
...
// client/src/views/customerpanel/CommercialProperties.jsx
import React, { useEffect, useState } from "react";
import { MapPin, Eye, Loader, Search, CheckCircle, Heart } from "lucide-react";
import API from "../../Api/axiosConfig";
...
export default function CommercialProperties() {
  ...
  const fetchCommercial = async () => {
    ...
    try {
      ...
      console.error("Commercial fetch error", err);
      setToast("Failed to load commercial properties");
  ...
  const toggleSave = async (id) => {
    ...
    try {
      ...
      setSavingMap((m) => { const copy = { ...m }; delete copy[id]; return copy; });
    }
  };

  if (loading) return <div className="min-h-screen flex items-center justify-center"><Loader className="animate-spin h-8 w-8 text-blue-600" /></div>;

  const list = commercial.filter((p) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="w-72 relative">
          <Search className="absolute left-3 top-3 h-4 w-4 text-gray-400" />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search commercial..." className="pl-10 pr-3 py-2 border rounded-md w-full" />
        </div>
      </div>
          ...
          <Card key={p._id} className="overflow-hidden">
            ...
            <CardContent>
              ...
              <div className="mt-3 flex items-center justify-between">
                <div className="text-lg font-bold">{formatCurrency(p.price)}</div>
                <div className="flex gap-2">
                  <Button onClick={() => toggleSave(p._id)} variant="ghost" size="sm" className="p-2">
                    {savedIds.includes(p._id) ? <span className="text-red-600 flex items-center gap-1"><CheckCircle className="h-4 w-4" />Saved</span> : <span><Heart className="h-4 w-4 text-gray-600" /> Save</span>}
                  </Button>
                  <Button onClick={() => window.alert("Open details")} variant="default" size="sm"><Eye className="h-4 w-4 mr-2" />View</Button>
                </div>
              </div>
